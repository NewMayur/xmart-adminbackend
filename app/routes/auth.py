...
from app.schema.Property import PropertyTypeMaster, Property
...
def all():
    ...
    print(students)
    if (students is not None) and bcrypt.check_password_hash(students.password, request.json["password"]):
        return response_base(message="Success", status=200)
    else:
